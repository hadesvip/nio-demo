...
/**
 * 直接缓冲:堆外内存
 * ByteBuffer.allocateDirect源码内部是new DirectByteBuffer(capacity)，
 * 构造方法里面有这段代码
 * //使用的sun包的方法，allocateMemory是用的native方法，使用的堆外内存(即操作系统内存)
 ...
 *
 * 为什么操作系统不直接操作jvm中的数据？
 * 因为jvm有一个垃圾回收，先标记后压缩（标记移动），如果操作系统正在操作buffer，发生了垃圾回收，数据就乱了
 *
